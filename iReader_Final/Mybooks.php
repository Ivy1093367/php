						...
						<div class="header_content d-flex flex-row align-items-center justify-content-start">
							...
							<nav class="main_nav_contaner ml-auto">
                                ...
                                <button type="button" class="btn btn-warning">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-book-half" viewBox="0 0 16 16" color="white">
										<path d="M8.5 2.687c.654-.689 1.782-.886 3.112-.752 1.234.124 2.503.523 3.388.893v9.923c-.918-.35-2.107-.692-3.287-.81-1.094-.111-2.278-.039-3.213.492V2.687zM8 1.783C7.015.936 5.587.81 4.287.94c-1.514.153-3.042.672-3.994 1.105A.5.5 0 0 0 0 2.5v11a.5.5 0 0 0 .707.455c.882-.4 2.303-.881 3.68-1.02 1.409-.142 2.59.087 3.223.877a.5.5 0 0 0 .78 0c.633-.79 1.814-1.019 3.222-.877 1.378.139 2.8.62 3.681 1.02A.5.5 0 0 0 16 13.5v-11a.5.5 0 0 0-.293-.455c-.952-.433-2.48-.952-3.994-1.105C10.413.809 8.985.936 8 1.783z"/>
									</svg>
									<font color="white">個人書櫃</font>
								</button>
		...
		<div class="container">
			<div class="row">
				<div class="col-lg-10 offset-lg-1">
					<div class="section_title text-center"><h2>趕快來看書不然要過期了!!</h2></div>
			...
			<div class="row courses_row">
				...
				if(isset($_COOKIE['uNo'])){
                    $uNo=$_COOKIE['uNo'];
                    $SQL="SELECT * FROM usersbook u,books b WHERE uNo=".$uNo." AND u.ISBN=b.ISBN";
                    if($result=mysqli_query($link,$SQL)){
                        while($row=mysqli_fetch_assoc($result)){
							...
							<div class="course">
								...
								<div class="course_body" height="50">
									<div class="course_header d-flex flex-row align-items-center justify-content-start">
										<div class="course_tag"><a href="book.php?isbn=<?php echo $row['ISBN']?>">開始閱讀</a></div>
									...
									<div class="course_footer d-flex align-items-center justify-content-start">
										...
										<div class="course_sales ml-auto"><?php
															if($row['time']==0){
																echo "實體書";
															}else if($row['time']==1){
																echo "電子書: <br/>無限期";
																...
																if($time>0){
																	echo "電子書時限:<br/>";
																	echo floor($time/86400)."日";
																	$time%=86400;
																	echo floor($time/3600)."時";
																	$time%=3600;
																	echo floor($time/60)."分";
																	echo floor($time%60)."秒";
																}else{
																	$is=$row['ISBN'];
																	$ti=$row['time'];
																	$SQL="DELETE FROM usersbook WHERE uNo='$uNo' AND ISBN='$is' AND time='$ti'";
	...
	<div class="courses">
		<div class="container">
			<div class="row">
				...
				<?php
					echo "<h2>實體書配送狀態:</h2><br/><br/><br/>";
					$SQL="SELECT DISTINCT OD.ISBN, O.sDelivery,b.bName FROM usersbook UB, orderdetail OD, orders O,books b WHERE UB.uNo=O.uNo AND O.oNo=OD.oNo AND OD.type LIKE 'RB' AND UB.uNo=".$uNo." AND b.ISBN=OD.ISBN";
					if($result=mysqli_query($link,$SQL)){
						echo "<table class='table caption-top table-hover'>";                        
						echo "<thead><tr><th>書本編號</th><th>書名</th><th>配送狀態</th></thead><tbody>";
						while($row=mysqli_fetch_assoc($result)){
							echo "<tr>";
							//echo "<td>".$row['ISBN']." ".$row['bName'].": ".$row['sDelivery']."</td>";
							echo "<td>".$row['ISBN']."</td><td>".$row['bName']."</td><td>".$row['sDelivery']."</td>";
							echo "</tr>";
							//echo "<br>";
						}
						echo "</tbody></table>";
