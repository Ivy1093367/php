		...
		<div class="container">
			<div class="row">
				<div class="col-lg-12 offset-lg-0">
                    ...
                    <p></p>
                    <div class="section_subtitle"></div><div>
                    <?php
                    $SQL="SELECT * FROM orders";
                    $de=array("處理中","寄出","取貨","退件","取消");
                    if($result=mysqli_query($link, $SQL)){
                        echo "<table class='table caption-top table-hover'>";                        
                        echo "<thead><tr><th>訂單編號</th><th>會員編號</th><th>收件人</th><th>電話</th><th>E-mail</th><th>訂單總額(明細)</th><th>付款方式</th><th>收件地址</th><th>訂單狀態</th><th></th><th></th></tr></thead><tbody>";
                        while($row=mysqli_fetch_assoc($result)){
                            echo "<tr>";
                            echo "<td>".$row['oNo']."</td>";
                            echo "<td>".$row['uNo']."</td>";
                            ...
                            echo "<td>";
                            //echo $row['amount'];
                            echo "<text class='tooltips'>".$row['amount'];
                            echo "<span class='tooltiptexts'>";
                            $SQL2="SELECT * FROM orderdetail WHERE oNo=".$row['oNo'];
                            if($result2=mysqli_query($link, $SQL2)){
                                while($row2=mysqli_fetch_assoc($result2)){
                                    echo "&nbsp;&nbsp;ISBN:".$row2['ISBN']." 數量:".$row2['num']." $".$row2['cost']."<br/>";
                                }
                            }
                            echo "</span></text>";
                            echo "</td>";
                            echo "<td>".$row['sPayment']."</td>";
                            echo "<td>".$row['sAddress']."</td>";
                            echo "<form action='' method='post'>";
                            echo "<td>";
                            echo "<select name='de'>";
                            echo"<option>".$row['sDelivery']."</option>";
                            foreach($de as $v){
                                if($v!=$row['sDelivery']){
                                    echo "<option>";
                                    echo $v;
                            ...
                            echo "<td><input type='submit' class='btn btn-info' value='修改'></td>";
                            echo "<td><a href='Admin_Odel.php?oNo=".$row['oNo']."'>刪除</a></td>";
                            echo "</form>";
                            echo "</tr>";
                        }
                        echo "</tbody></table>";
                    ...
                    if(isset($_POST['de'])){
                        $d=$_POST['de'];
                        $o=$_POST['oNo'];
                        $SQL="UPDATE orders SET sDelivery='$d' WHERE oNo=".$o;
