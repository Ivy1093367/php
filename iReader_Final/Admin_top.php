		...
		<div class="container">
			<div class="row">
				<div class="col-lg-10 offset-lg-1">
                    ...
                    <?php
                    $SQL="SELECT TB.rank, TB.ISBN, B.launch FROM topbooks TB, books B WHERE TB.ISBN=B.ISBN";
                    if($result=mysqli_query($link, $SQL)){
                        echo "<table class='table caption-top table-hover'>";
                        echo "<thead><tr><th>Rank</th><th>商品圖</th><th>ISBN</th><th></th></tr></thead><tbody>";
                        while($row=mysqli_fetch_assoc($result)){
                            echo "<tr>";
                            echo "<td>".$row['rank']."</td>";
                            echo "<td><img style='width:200px;' src='images/".$row['ISBN'].".png'></td>";
                            if($row['launch']){
                                echo "<td>".$row['ISBN']."</td>";
                            }else{
                                echo "<script type='text/javaScript'>";
                                echo "alert('注意! ".$row['ISBN']."未上架');";
                                echo "</script>";
                                echo "<td><font color=red>".$row['ISBN']."</font></td>";
                            }
                            echo "<td><a href='' onclick='writeInfo(".$row['rank'].",".$row['ISBN'].")'>修改</a></td>";
                            echo "</tr>";
                        }
                        echo "</tbody></table>";
                    }
...
<script>
        function writeInfo(rank, isbn) {
            isbn = prompt("請輸入ISBN");
                if (isbn != null) {
                    alert("更改為"+isbn);
                    window.location.href = "Admin_Tcha.php?rank="+rank+"&isbn="+isbn; 
                }
        }
</script>
